...
def send_invoices():
    ...
    for client in clients_in_need:
        ...
        if summary.work_hours > 0:
            ...
            input('OK?')
            toggl_pdf = toggl.get_summary_pdf(client.workspace_id, client.client_id, since=invoice_since, until=invoice_until)
            print(f'PDF: {toggl_pdf}')
            subprocess.call(['xdg-open', toggl_pdf])
            ...
            xero_pdf = xero.get_pdf(invoice)
            import pdb; pdb.set_trace()
            gmail.send_invoice(client, invoice_since, invoice_until, link, toggle_pdf_path=toggl_pdf, xero_pdf_path=xero_pdf)
